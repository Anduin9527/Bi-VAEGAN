...
class norm(nn.Module):
    ...
    def forward(self, x):
        x = self.radius * x / torch.norm(x, p=2, dim=-1, keepdim=True)  # L2 norm
        return x
...
class Encoder(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.MLP = nn.Sequential(
            *[
                nn.Linear(opt.resSize + opt.attSize, 4096),
                nn.BatchNorm1d(4096),
                nn.LeakyReLU(0.2, True),
            ]
        )
